...
Adds new Timeline Delta, Death Replay, Moments History, Achievement Badges, and Admin Player Lookup GUIs - players can now compare their weekly progress ("This week vs last week"), review their death history with detailed information, browse their achievements/moments, view earned badges with progress tracking, and admins can search and view any player's stats.
...
- New "🏆 Moments" button in PlayerStatsGui (slot 48)
...
- New "🏅 Badges" button in PlayerStatsGui (slot 50)
...
- Five tier levels with visual distinction:
  ...
  - 🥇 Gold - Significant accomplishments
  - 💎 Diamond - Major achievements
  - 🌟 Legendary - Ultimate milestones
- Seven badge categories:
  - ⚔ Combat (kills, damage dealt, PvP)
  - 🧭 Exploration (distance traveled, biomes discovered)
  - ⛏ Mining (blocks broken)
  - 🏗 Building (blocks placed, crafting)
  - 👥 Social (balanced activity)
  - 💀 Survival (death avoidance, damage tanked)
  - ⏱ Dedication (playtime milestones)
- Features:
...
### Admin Player Lookup GUI
- New admin-only GUI accessible from Main Menu (slot 31)
- Search and view any player's stats (requires `smpstats.admin` permission)
- Features:
  - 🔍 Player search with name filtering
  - 📊 Multiple sort modes:
    - ⏱ Playtime (descending)
    - 📛 Name (alphabetical)
    - 📅 Last Seen (most recent first)
    - ⚔ Kills (highest first)
    - 💀 Deaths (highest first)
  - Pagination (28 players per page)
  - Online/offline status indicators (green/gray)
  - Quick stats preview per player (playtime, kills, deaths)
  - Click to view full stats of any player
  - Refresh button to reload player data
- Player heads displayed with online status icons
- Relative "last seen" timestamps for offline players
...
- Admin Player Lookup accessible from MainMenuGui via "Player Lookup" button (slot 31, admin only)
...
### Admin Player Lookup GUI
- New `AdminPlayerLookupGui` class in `de.nurrobin.smpstats.gui`
- Uses `StatsStorage.loadAll()` for loading all player records
- Sort mode enum for multiple sorting strategies
- Permission check for `smpstats.admin`
...
- Admin Player Lookup requires `smpstats.admin` permission

---

## 🗺️ Roadmap

This release completes five items from the GUI Enhancement Roadmap (Milestone 2):
- ✅ Timeline Delta View (Medium feature)
- ✅ Death Replay GUI (Larger feature)
- ✅ Moments/Achievements Browser (Larger feature)
- ✅ Achievement Badges (Medium feature)
- ✅ Admin Player Lookup (Larger feature)
